...
import com.aliyuncs.nas.model.v20170626.DescribeTagsResponse.Tag;
...
public class DescribeTagsResponseUnmarshaller {
	...
	public static DescribeTagsResponse unmarshall(DescribeTagsResponse describeTagsResponse, UnmarshallerContext _ctx) {
		...
		for (int i = 0; i < _ctx.lengthValue("DescribeTagsResponse.Tags.Length"); i++) {
			...
			tag.setValue(_ctx.stringValue("DescribeTagsResponse.Tags["+ i +"].Value"));

			List<String> fileSystemIds = new ArrayList<String>();
			for (int j = 0; j < _ctx.lengthValue("DescribeTagsResponse.Tags["+ i +"].FileSystemIds.Length"); j++) {
				fileSystemIds.add(_ctx.stringValue("DescribeTagsResponse.Tags["+ i +"].FileSystemIds["+ j +"]"));
			}
			tag.setFileSystemIds(fileSystemIds);
