...
public class DefaultAcsClient implements IAcsClient {
    ...
    public <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, String regionId, Credential credential)
        ...
        return doAction(request, autoRetry, maxRetryNumber, regionId, new LegacyCredentials(credential), format);
    }
    ...
    public <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, boolean autoRetry, int maxRetryCounts,
        ...
        return this.doAction(request, retry, retryNumber, request.getSysRegionId(), credentials, format);
    }
    ...
    public <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, boolean autoRetry, int maxRetryNumber,
            ...
            throws ClientException, ServerException {
        return doAction(request, autoRetry, maxRetryNumber, regionId, new LegacyCredentials(credential), signer, format);
    }
    ...
    private <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, boolean autoRetry, int maxRetryNumber,
                                                          String regionId, AlibabaCloudCredentials credentials, FormatType format)
            throws ClientException, ServerException {
        String signatureMethod = this.getProfile().getSignatureMethod();
        Signer signer;
        if (signatureMethod == null || signatureMethod.isEmpty()) {
            signer = Signer.getSigner(credentials);
        } else {
            signer = Signer.getSigner(signatureMethod);
        }
        return doAction(request, autoRetry, maxRetryNumber, regionId, credentials, signer, format);
    }
