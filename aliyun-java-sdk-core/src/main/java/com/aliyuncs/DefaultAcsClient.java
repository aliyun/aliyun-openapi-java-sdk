...
public class DefaultAcsClient implements IAcsClient {
    ...
    public <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, String regionId, Credential credential)
            throws ClientException, ServerException {
        String signatureMethod = this.getProfile() != null && this.getProfile().getSignatureMethod() != null
                ? this.getProfile().getSignatureMethod() : "";
        Signer signer = Signer.getSigner(new LegacyCredentials(credential), signatureMethod);
    ...
    public <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, boolean autoRetry, int maxRetryCounts,
        ...
        String signatureMethod = this.getProfile() != null && this.getProfile().getSignatureMethod() != null
                ? this.getProfile().getSignatureMethod() : "";
        Signer signer = Signer.getSigner(credentials, signatureMethod);
        FormatType format = profile.getFormat();
        if (profile.getSignatureMethod() != null && !profile.getSignatureMethod().isEmpty()) {
